...
<head>
    <meta charset="UTF-8">
    <title>SLIP GAJI MASSAL - {{ $payroll->period ?? '' }} ({{ $payroll->payroll_code ?? '' }})</title>
    <style>
        body {
...
<body>
    <div class="no-print" style="margin-bottom: 15px; text-align: right;">
        <button onclick="window.print()" style="padding: 10px 20px; background: #0284c7; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer;">
            Cetak / Download Semua Slip PDF ({{ count($payroll->items) }} Karyawan)
        </button>
    </div>

    @foreach($payroll->items as $index => $item)
        <div class="wrapper {{ $index < count($payroll->items) - 1 ? 'page-break' : '' }}">
            <!-- Header Toko / Cabang -->
            <div class="header">
                <div>
                    <div class="store-name">{{ $item->branch->store_name ?? ($payroll->branch->store_name ?? 'DIEGO MUSIC STORE') }}</div>
                    <div class="store-sub">{{ $item->branch->name ?? ($payroll->branch->name ?? 'Cabang Utama') }}</div>
                </div>
                <div class="title">
                    <h2>SLIP GAJI KARYAWAN</h2>
                    <p>Kode Ref: {{ $payroll->payroll_code ?? '-' }}</p>
