...
{
    /**
     * Execute the POS Update Checkout process.
     *
     * @param  Sale   $sale
     * @param  array  $data
     * @return Sale
    ...
    public function execute(Sale $sale, array $data): Sale
    {
        return DB::transaction(function () use ($sale, $data) {
            ...
            $branchId = $sale->branch_id; // Keep branch unchanged
            $salesRepId = $data['sales_rep_id'] ?? $sale->sales_rep_id;
            ...
            foreach ($sale->items as $item) {
                ...
                if ($variant && $variant->product->isPhysical()) {
                    $branchStock = ProductBranchStock::where('product_variant_id', $variant->id)
            ...
            // 2. Delete Old Stock Movements
            StockMovement::where('reference_type', 'POS')
                ->where('reference_id', $sale->id)
                ->delete();

            // 3. Delete Old Journal Entries & Items
            $oldJournalEntries = JournalEntry::where('reference_type', 'Sales')
                ->where('reference_id', $sale->id)
                ->get();
            foreach ($oldJournalEntries as $je) {
                $je->items()->delete();
                $je->delete();
            }
            ...
            // 6. Update Sale Header (keep invoice number unchanged)
            $sale->update([
            ...
            // 7. Create New Sale Items and handle Stock Deduction & Movement
            $totalCOGS = 0;
            ...
            $journalEntry = JournalEntry::create([
                ...
                'description' => "Penjualan Kasir POS (Update): No. Invoice {$sale->invoice_number}",
