@props(['variant', 'selectedBranchId', 'selectedPricingTierId' => null, 'qtyInCart' => 0])
...
<div 
    @if (!$isOutOfStock) wire:click="addToCart({{ $variant->id }})" @endif 
    class="p-3 rounded-2xl shadow-sm border transition-all flex flex-col justify-between {{ $isOutOfStock ? 'opacity-40 saturate-50 bg-slate-100/60 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700 pointer-events-none cursor-not-allowed select-none' : ($qtyInCart > 0 ? 'bg-blue-50/30 dark:bg-blue-950/20 border-primary dark:border-blue-500 shadow-blue-500/5 cursor-pointer group' : 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 hover:shadow-md hover:border-primary dark:hover:border-blue-500 cursor-pointer group') }}"
>
    <div>
