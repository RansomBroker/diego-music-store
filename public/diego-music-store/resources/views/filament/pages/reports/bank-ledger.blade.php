            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-sm tracking-wide">
                        {{ $data['selected_account'] ? ('BUKU BANK: ' . $data['selected_account']->code . ' - ' . $data['selected_account']->name) : 'REKAPITULASI SELURUH REKENING BANK' }}
                    </h4>
                    <p class="text-xs opacity-80">
                        Periode: {{ \Illuminate\Support\Carbon::parse($data['from_date'])->translatedFormat('d F Y') }} s.d. {{ \Illuminate\Support\Carbon::parse($data['to_date'])->translatedFormat('d F Y') }} • Cabang: {{ $data['branch_name'] }} • Mutasi: {{ strtoupper($data['mutation_type']) }}
                    </p>
                </div>
            </div>
            <div class="text-right flex items-center gap-6">
                <div>
                    <span class="text-xs font-medium uppercase opacity-75">Total Uang Masuk</span>
                    <div class="text-sm font-extrabold font-mono text-gray-900 dark:text-white">{{ \App\Helpers\FinancialReportHelper::formatRupiah($data['grand_total_in']) }}</div>
                </div>
                <div>
                    <span class="text-xs font-medium uppercase opacity-75">Total Uang Keluar</span>
                    <div class="text-sm font-extrabold font-mono text-gray-900 dark:text-white">{{ \App\Helpers\FinancialReportHelper::formatRupiah($data['grand_total_out']) }}</div>
                </div>
            </div>
        </div>
    </div>

    {{-- Bank Ledger Accounts List --}}
    <div class="space-y-6">
                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-800 -mx-6 -mb-6 border-t border-gray-200 dark:border-white/10">
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="bg-gray-100/70 dark:bg-white/5 text-gray-700 dark:text-gray-300 text-xs uppercase font-bold border-b border-gray-300 dark:border-gray-700">
                                <th class="py-2.5 px-6 w-28">Tanggal</th>
                                <th class="py-2.5 px-6 w-36">No. Bukti Transaksi</th>
                                <th class="py-2.5 px-6">Keterangan / Deskripsi Transaksi</th>
                                <th class="py-2.5 px-6 text-right w-36">Uang Masuk (Debit)</th>
                                <th class="py-2.5 px-6 text-right w-36">Uang Keluar (Kredit)</th>
                                <th class="py-2.5 px-6 text-right w-40">Running Balance</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr class="bg-gray-50/80 font-bold dark:bg-white/5 text-gray-900 dark:text-white">
                                ...
                                <td class="py-2.5 px-6 font-semibold uppercase text-xs tracking-wider">
                                    SALDO AWAL REKENING BANK (BEGINNING BALANCE)
                                </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-3 px-6 text-center text-gray-400 text-xs italic">
                                        Tidak ada mutasi rekening bank pada periode ini.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    ...
                    <div class="p-3.5 px-6 bg-gray-100 dark:bg-white/10 flex justify-between items-center text-gray-900 dark:text-white">
                        <div class="flex items-center gap-6">
                            <span class="font-bold text-xs uppercase tracking-wider">TOTAL MUTASI REKENING</span>
                            <span class="text-xs font-mono">Uang Masuk: <strong>{{ \App\Helpers\FinancialReportHelper::formatRupiah($ledger['total_in']) }}</strong></span>
                            <span class="text-xs font-mono">Uang Keluar: <strong>{{ \App\Helpers\FinancialReportHelper::formatRupiah($ledger['total_out']) }}</strong></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-xs uppercase tracking-wider">SALDO AKHIR BANK:</span>
        ...
        @empty
            <div class="p-12 text-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm">
                ...
                <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">Tidak Ada Rekening Bank Ditemukan</p>
                <p class="text-xs text-gray-400 mt-1">Silakan sesuaikan filter rekening bank atau tanggal pada form di atas.</p>
            </div>
        @endforelse
    </div>
</x-filament-panels::page>
