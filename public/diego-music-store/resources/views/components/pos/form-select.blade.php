...
<div>
    <label class="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1.5">{{ $label }}</label>
    <div class="relative">
        @if ($icon)
            <i class="ph {{ $icon }} text-slate-400 dark:text-slate-500 absolute left-4 top-1/2 -translate-y-1/2 text-lg pointer-events-none"></i>
        @endif
        <select 
            wire:model="{{ $model }}" 
