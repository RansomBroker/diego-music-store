...
>
    ...
    <div class="mb-6">
        ...
        <div class="grid grid-cols-3 gap-3">
            <button wire:click="setPaymentMethod('cash')" class="flex flex-col items-center justify-center p-4 border-2 rounded-2xl transition-all {{ $paymentMethod === 'cash' ? 'border-primary bg-primary-light/35 text-primary dark:border-blue-500 dark:bg-blue-950/20 dark:text-blue-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700' }}">
                <i class="ph-fill ph-money text-2xl mb-1.5"></i>
                <span class="text-xs font-bold">Tunai</span>
            </button>
            <button wire:click="setPaymentMethod('debit')" class="flex flex-col items-center justify-center p-4 border-2 rounded-2xl transition-all {{ $paymentMethod === 'debit' ? 'border-primary bg-primary-light/35 text-primary dark:border-blue-500 dark:bg-blue-950/20 dark:text-blue-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700' }}">
                <i class="ph-fill ph-credit-card text-2xl mb-1.5"></i>
                <span class="text-xs font-bold">Debit BCA</span>
            </button>
            <button wire:click="setPaymentMethod('credit')" class="flex flex-col items-center justify-center p-4 border-2 rounded-2xl transition-all {{ $paymentMethod === 'credit' ? 'border-primary bg-primary-light/35 text-primary dark:border-blue-500 dark:bg-blue-950/20 dark:text-blue-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-355 hover:bg-slate-50 dark:hover:bg-slate-700' }}">
    ...
    @if ($paymentMethod === 'cash')
        <div class="mb-6">
            ...
            <div class="relative">
                ...
                <input type="number" wire:model.live="amountPaid" class="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none font-bold text-lg focus:ring-2 focus:ring-primary-light dark:focus:ring-blue-950 text-slate-800 dark:text-slate-100" placeholder="0">
            </div>
        </div>
